...
if __name__ == '__main__':
    for prov_name in prov_list_epidemics:
        ...
        calibrated_params, ics = model_params.use_calibrated_params(prov=prov_name,file=prov_file)
        params = model_params(**calibrated_params)
        it_dict = model_params.initialise_dict()

        treatment_scenarios = pN_vec #model_params.pN_vec
        coverage_scenarios = c_vec #model_params.c_vec

        #for iterate1 in range(len(pN_vec)):
        for iterate_treat in range(len(treatment_scenarios)):
            # treatment scenarios
            ...
            for iterate_cov in range(len(coverage_scenarios)):
                ...
                ####
                it_dict = model_params.update_dict(params, it_dict, iterate_treat, iterate_cov)
                ###
